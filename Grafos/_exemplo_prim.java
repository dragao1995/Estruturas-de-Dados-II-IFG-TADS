...
public class _exemplo_prim {
	...
	private static void djikstra(List<List<Integer>> g, List<List<Integer>> w,
			int v) {
		List<Integer> color = new ArrayList<Integer>(Collections.nCopies(
				g.size(), 0));
		List<Integer> distancia = new ArrayList<Integer>(Collections.nCopies(
		...
		while (!q.isEmpty()) {
			...
			for (int i = 0; i < g.get(u).size(); i++) {
				...
				if (color.get(z) != 2 && distancia.get(z) > peso_aresta) {
					distancia.set(z, peso_aresta);
